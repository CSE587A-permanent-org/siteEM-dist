...
from siteEM.utils.consensus_pfm import consensus_pfm
from siteEM.assignment import (e_step,
                               update_motif_prior,
                               update_site_probs,
                               m_step,
                               siteEM,
...
class TestSiteEM(unittest.TestCase):
    ...
    def test_3(self):
        """
        The window posteriors should high, since the lerned motif is a good
...
class TestSiteEMInitializer(unittest.TestCase):
    def setUp(self) -> None:
        # read in fasta files
        with path('siteEM.data', 'tinyTest.fasta') as tiny_fasta_path:
            self.tiny_fasta_path = tiny_fasta_path
        with path('siteEM.data', 'smallTest.fasta') as small_fasta_path:
            self.small_fasta_path = small_fasta_path
        with path('siteEM.data', 'PACPlusSeqs.fasta') as pac_fasta_path:
            self.pac_fasta_path = pac_fasta_path
    @weight(2)
